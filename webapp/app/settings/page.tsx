...
export default function Settings(){
  const [neo,setNeo]=useState(false); const [accent,setAccent]=useState(0); const [font,setFont]=useState(1); const [notif,setNotif]=useState(true);
  useEffect(()=>{
    const s=localStorage.getItem("vastavik_settings"); if(s){ try{ const j=JSON.parse(s); setNeo(!!j.neo); setAccent(j.accent||0); setFont(j.font||1); setNotif(j.notif!==false);}catch{} }
  },[]);
  useEffect(()=>{
    localStorage.setItem("vastavik_settings", JSON.stringify({neo,accent,font,notif}));
  ...
  },[neo,accent,font,notif]);

  const Toggle = ({on, set}:{on:boolean; set:()=>void}) => (
    <button onClick={set} className={`w-[52px] h-[30px] rounded-full p-1 transition flex items-center ${on?"bg-ink":"bg-slate-200"}`} aria-pressed={on}>
      <span className={`block w-[22px] h-[22px] bg-white rounded-full shadow-soft transition ${on?"translate-x-[22px]"}`} />
    </button>
  );
  ...
  return (
    <div className="space-y-5 max-w-lg mx-auto">
        ...
        <div className="divide-y divide-slate-100">
          ...
          {neo && (
            <div className="p-4 bg-amber-50/50">
              ...
              <div className="mt-3 flex gap-2.5 flex-wrap">{accents.map((c,i)=> <button key={c} onClick={()=>setAccent(i)} className={`w-12 h-12 rounded-full border-[3px] flex items-center justify-center font-bold shadow-soft transition ${accent===i?"border-ink scale-105":"border-transparent"}`} style={{ background:c }}>{accent===i?"✓":""}</button>)}</div>
            </div>
          )}
      ...
      <div className="neo divide-y overflow-hidden">
        <Link href="/notifications" className="flex justify-between items-center p-4 hover:bg-slate-50 transition"><span className="font-medium text-sm">Notification History</span><span className="text-muted">›</span></Link>
        <Link href="/app-update" className="flex justify-between items-center p-4 hover:bg-slate-50 transition"><div><p className="font-medium text-sm">App Update</p><p className="text-xs text-muted">Check for updates</p></div><span className="text-muted">›</span></Link>
